...
class Board:
    ...
    def main(self):
        ...
        #Image of the Queen
        self.queenImg = pygame.image.load('QueenImage.png')
        ...
        for row in range(boardLength):
            ...
            #check if current loop value is even
                if(self.board[row][col].isWhite()):
                    pygame.draw.rect(self.gameDisplay, white, [size*(row + 1),size*(col + 1),size,size])
                else:
                    pygame.draw.rect(self.gameDisplay, gray, [size*(row + 1),size*(col + 1),size,size])
                if(self.board[row][col].isQueen()):
                    self.gameDisplay.blit(self.queenImg,(size*(row + 1),size*(col + 1)))

        #border
        pygame.draw.rect(self.gameDisplay,black,[size,size,boardLength*size,boardLength*size],2)
        pygame.draw.rect(self.gameDisplay,black,[size-10,size-10,boardLength*size + 20,boardLength*size + 20],2)
